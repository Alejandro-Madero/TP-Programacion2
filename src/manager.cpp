...
int Manager::agregarOrdenProduccion(std::string codigo, int cantidad)
{
   ...
   if (!this->getComposicionProducto(posProducto, vectorInsumo, composicionSize, vectorComposicion, codigo))
      return -3;
   ...
   for (int i = 0; i < composicionSize; i++)
   {
      if (vectorInsumo[i].getStock() < vectorComposicion[i] * cantidad)
         return -4;
   }
...
bool Manager::modificarEstadoProduccion(int pos, std::string estado)
{
   ...
   if (orden.getEstado() != "I")
      return false;
   if (estado != "T" && estado != "C")
      return false;
   ...
   if (!this->archivoOrdenProduccion.Guardar(orden, pos))
      return false;
   if (!this->archivoRecurso.Guardar(producto, posProducto))
      return false;
   return true;
}
